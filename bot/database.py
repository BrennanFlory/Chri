...
class Database():
    ...
    async def search_table(self, table, item1, item2):
        """
        Searches a table in the database
        """
        try:
            ids = r.table(table).filter(lambda warning1: warning1[item1].match(item2)).run(self.db)
            log.info("Searched table: {} and found {}".format(table, ids))
        except r.errors.ReqlOpFailedError:
